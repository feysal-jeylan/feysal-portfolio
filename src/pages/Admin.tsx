...
import { Loader2, Plus, Pencil, Trash2, LogOut, Eye, EyeOff, ArrowLeft } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="glass-card p-8 w-full max-w-sm">
        ...
        <Link to="/" className="text-xs text-muted-foreground hover:text-primary mt-4 block text-center">
          ← Back to site
        </Link>
      </div>
    </div>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <input value={excerpt} onChange={e => setExcerpt(e.target.value)} placeholder="Excerpt / Preview text"
        className="w-full px-4 py-3 rounded-lg bg-secondary/50 border border-border text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary/50" />
      <textarea value={content} onChange={e => setContent(e.target.value)} placeholder="Content (Markdown supported)" rows={8}
      ...
      <div className="flex gap-3">
        ...
        <button type="button" onClick={onCancel} className="px-6 py-2 rounded-lg border border-border text-foreground text-sm hover:bg-secondary transition-colors">
          Cancel
        </button>
      </div>
    </form>
...
function TestimonialForm({ testimonial, onSave, onCancel }: { testimonial?: Testimonial | null; onSave: () => void; onCancel: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = testimonial
      ? await supabase.from("testimonials").update(data).eq("id", testimonial.id)
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        ...
        <input value={photo} onChange={e => setPhoto(e.target.value)} placeholder="Photo URL"
          className="px-4 py-3 rounded-lg bg-secondary/50 border border-border text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 sm:col-span-2" />
      </div>
      <label className="flex items-center gap-2 text-sm text-foreground cursor-pointer">
        ...
        {dataLoading ? (
          ...
          <div>
            ...
            <div className="space-y-3">
              ...
              {testimonials.length === 0 && <p className="text-sm text-muted-foreground text-center py-8">No testimonials yet. Create your first one!</p>}
            </div>
          </div>
        )}
      </div>
